...
// order in which to save row data
...
// total num of keys in given node
uint32_t* internal_node_num_keys(void* node) {
  return node + INTERNAL_NODE_NUM_KEYS_OFFSET;
}

// return first right child of a given node
...
// return cell (key + value) correspond to given key
...
uint32_t internal_node_find_child(void* node, uint32_t key) {
  ...
  // compare as long as a key remains to be compared
  // if  min_index == max_index, there is only one key
...
// identifiy the type of node and return value correspond to given value
// if leaf node, return a cell of given key of given node
// if internal node, return child node of given key of given node
...
Pager* pager_open(const char* filename) {
  ...
  pager->num_pages = (file_length / PAGE_SIZE); //total nb of page in a file
...
void create_new_root(Table* table, uint32_t right_child_page_num) {
  ...
  *internal_node_right_child(root) = right_child_page_num;
  //left child parent is root node
  *node_parent(left_child) = table->root_page_num;
  //right child parent is root node
...
void leaf_node_split_and_insert(Cursor* cursor, uint32_t key, Row* value) {
  ...
  */
  ...
  uint32_t old_max = get_node_max_key(old_node); // last key in a node
...
void leaf_node_insert(Cursor* cursor, uint32_t key, Row* value) {
  ...
  if (cursor->cell_num < num_cells) {
    ...
    // the last cell is blank so we can move a cell
